...
public class Exercise2Test extends ClassicOnlineStore {
    ...
    public void descSortByAge() {
         ...
         */
        Comparator<Integer> descOrder = comparingInt(o -> o);
        Stream<Integer> sortedAgeStream = customerList.stream().map(Customer::getAge).sorted(descOrder.reversed());
    ...
    public void top3RichCustomer() {
         ...
         */
        Stream<String> top3RichCustomerStream = customerList.stream().sorted(comparingInt(Customer::getBudget).reversed()).map(Customer::getName).limit(3);
