...
Exercise9Test extends ClassicOnlineStore {
    ...
    public void simplestStringJoin() {
        ...
        BiConsumer<StringBuilder, String> accumulator = (o, s) -> o.append(s).append(",");
        BinaryOperator<StringBuilder> combiner = (o, o2) -> o.append(o2);
        Function<StringBuilder, String> finisher = s -> s.deleteCharAt(s.length() - 1).toString();
    ...
    public void mapKeyedByItems() {
         ...
         */
        Supplier<Map<String, Set<String>>> supplier = () -> new ConcurrentHashMap<>();
        BiConsumer<Map<String, Set<String>>, Customer> accumulator = (map, customer) -> customer.getWantToBuy().forEach(item -> map.compute(item.getName(), (k, v) -> {
            v = v == null ? new HashSet<String>() : v;
            v.add(customer.getName());
            return v;
        }));
        BinaryOperator<Map<String, Set<String>>> combiner = (map1, map2) -> {
            map1.forEach((item, customers) -> map2.merge(item, customers, (customers1, customers2) -> { customers1.addAll(customers2); return customers1 ;}));
            return map2;
        };
        Function< Map<String, Set<String>>, Map<String, Set<String>>> finisher = o -> o;
    ...
    public void bitList2BitString() {
         ...
         */
        Collector<String, ?, String> toBitString =  new CollectorImpl<>(() -> new StringBuilder(), (sb, s) -> sb.append(s), (st, st2) -> st.append(st2), stringBuilder -> stringBuilder.toString(), Collections.emptySet());
