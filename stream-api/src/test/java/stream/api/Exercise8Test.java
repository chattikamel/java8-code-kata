...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void itemsNotOnSale() {
         ...
         */
        List<String> itemListOnSale = shopStream.flatMap(shop -> shop.getItemList().stream()).map(Item::getName).distinct().collect(Collectors.toList());
        Set<String> itemSetNotOnSale = customerStream.flatMap(customer -> customer.getWantToBuy().stream()).map(Item::getName).filter(s -> !itemListOnSale.contains(s)).distinct().collect(Collectors.toSet());
    ...
    public void havingEnoughMoney() {
        ...
        Predicate<Customer> havingEnoughMoney = customer -> onSale.stream().filter(item -> customer.getWantToBuy().contains(item)).map(item -> item.getPrice()).mapToLong(value -> value).sum() < customer.getBudget();
